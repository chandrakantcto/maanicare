                        ...
                        <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" id="project-form">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <small class="text-muted">Slug: <strong>{{ $project->slug }}</strong></small>
                                        @error('title')
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('sector') is-invalid @enderror"
                                            id="sector" name="sector" value="{{ old('sector', $project->sector) }}">
                                        @error('sector')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-2">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control @error('area_sqft') is-invalid @enderror"
                                            id="area_sqft" name="area_sqft" value="{{ old('area_sqft', $project->area_sqft) }}" min="0" step="1">
                                        @error('area_sqft')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-2">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('area_display') is-invalid @enderror"
                                            id="area_display" name="area_display" value="{{ old('area_display', $project->area_display) }}" placeholder="e.g. 25,000 Sq. Ft.">
                                        @error('area_display')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('special_features') is-invalid @enderror"
                                            id="special_features" name="special_features" rows="3">{{ old('special_features', $project->special_features) }}</textarea>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('key_highlights') is-invalid @enderror"
                                            id="key_highlights" name="key_highlights" rows="3">{{ old('key_highlights', $project->key_highlights) }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('services') is-invalid @enderror"
                                    id="services" name="services" rows="3" placeholder="One service per line">{{ old('services', $project->services ? implode("\n", $project->services) : '') }}</textarea>
                                <small class="text-muted">One service per line.</small>
                                @error('services')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        @if ($project->hero_image)
                                            <div class="mt-2">
                                                <img src="{{ asset('storage/' . $project->hero_image) }}" alt="" class="rounded" style="max-height: 80px;">
                                                <small class="text-muted d-block">Current. Upload to replace.</small>
                                            </div>
                                        @else
                            ...
                            <div id="images-container">
                                @foreach($project->images as $idx => $img)
                                    <div class="image-row card mb-2" data-index="{{ $idx }}">
                                        <div class="card-body py-2">
                                            <input type="hidden" name="images[{{ $idx }}][id]" value="{{ $img->id }}">
                                            <div class="row align-items-center">
                                                <div class="col-md-3">
                                                    <input type="file" class="form-control form-control-sm" name="images[{{ $idx }}][image]" accept="image/*">
                                                    @if ($img->image_path)
                                                        <img src="{{ asset('storage/' . $img->image_path) }}" alt="" class="rounded mt-1" style="max-height: 50px;">
                                                        <small class="text-muted">Upload to replace</small>
                                                    @endif
                                                </div>
                                                <div class="col-md-3">
                                                    <input type="text" class="form-control form-control-sm" name="images[{{ $idx }}][caption]" value="{{ old('images.'.$idx.'.caption', $img->caption) }}" placeholder="Caption">
                                                </div>
                                                <div class="col-md-1">
                                                    <input type="number" class="form-control form-control-sm" name="images[{{ $idx }}][sort_order]" value="{{ old('images.'.$idx.'.sort_order', $img->sort_order) }}" min="0">
                                                </div>
                                                <div class="col-md-2">
                                                    <select class="form-select form-select-sm" name="images[{{ $idx }}][status]">
                                                        <option value="Active" {{ ($img->status ?? 'Active') == 'Active' ? 'selected' : '' }}>Active</option>
                                                        <option value="InActive" {{ ($img->status ?? '') == 'InActive' ? 'selected' : '' }}>InActive</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="images[{{ $idx }}][is_hero]" value="1" {{ $img->is_hero ? 'checked' : '' }}>
...
@section('scripts')
    ...
    (function() {
        ...
        var index = {{ $project->images->count() }};
