    ...
    <script>
        (function() {
            document.addEventListener('submit', function(e) {
                ...
                if (method === 'GET') {
                    url = url + (url.indexOf('?') === -1 ? '?' : '&') + new URLSearchParams(formData)
                .toString();
                    formData = null;
                }
                ...
                fetch(url, {
                    ...
                    .then(function(result) {
                        var msg = result.data && result.data.message ? result.data.message : (form
                            .getAttribute('data-success-message') || 'Submitted successfully.');
                        if (result.ok && (result.data === undefined || result.data.success !== false)) {
                            ...
                            var errMsg = (result.data && result.data.message) ? result.data.message : (
                                result.data && result.data.errors ? Object.values(result.data.errors)
                                .flat().join(' ') : 'Something went wrong. Please try again.');
                            if (messageEl) {
                                messageEl.style.display = 'block';
